...
def test_classify(tree, testing_set):
    result = []
    result_prob = []
    for row in testing_set:
        category = classify(row, tree)
        max_cat = None
        max_prob = 0
        for (cat, prob) in category.items():
            if int(prob[0:-1]) > max_prob:
                max_cat = cat
                max_prob = int(prob[0:-1])
        result.append(max_cat)
        result_prob.append(max_prob)
